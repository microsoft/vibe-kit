...
def test_uninstall_removes_customizations(run_cli, tmp_path: Path):
    ...
    (repo / "customizations" / "b.agent.md").write_text("Agent B")
    ...
    chat_file = state / "agents" / "b.agent.md"
    instructions_file = state / "instructions" / "c.instructions.md"
    assert prompt_file.exists()
    assert chat_file.exists()
    assert instructions_file.exists()

    result = run_cli(tmp_path, "uninstall", "u-kit", "--yes")

    print(result.stdout)
    assert result.returncode == 0, result.stdout + result.stderr
    assert "removed 3 customization assets" in result.stdout.lower()
