...
def test_list_available_and_installed(run_cli, tmp_path: Path):
    ...
    lines = [l for l in res_avail_json.stdout.splitlines() if l.strip()]
    if lines and lines[0].startswith("Repository source:"):
        json_text = "\n".join(lines[1:])
    else:
        json_text = res_avail_json.stdout
    avail = json.loads(json_text)
