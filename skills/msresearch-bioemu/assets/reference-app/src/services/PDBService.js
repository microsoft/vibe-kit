...
const getBackendUrl = () => {
  ...
  // If legacy API URL is set, use it
  if (process.env.REACT_APP_API_URL) {
    return process.env.REACT_APP_API_URL;
  }
  
  // Auto-detect based on current domain for Azure App Service deployments
...
export const fetchPDBSequence = async (pdbId, chainId = null) => {
  try {
    ...
    console.error(`❌ Error fetching PDB sequence for ${pdbId}:`, error);
    return {
      success: false,
...
export const fetchPDBInfo = async (pdbId) => {
  try {
    ...
    console.error(`❌ Error fetching PDB info for ${pdbId}:`, error);
    return {
      success: false,
...
export const fetchPDBChains = async (pdbId) => {
  try {
    ...
    console.error(`❌ Error fetching PDB chains for ${pdbId}:`, error);
    return {
      success: false,
