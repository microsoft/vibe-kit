...
import MolstarViewerFixed from './MolstarViewerTrajectoryControl.working';
...
const ConformationalExplorer = ({ 
  ...
  const calculatePCA = useMemo(() => {
    try {
      ...
      const nFeatures = contactFeatures[0].length;
      ...
      // Only log ranges if we have data
      if (pcaData.length > 0) {
      }
    ...
    if (frameData) {
      ...
      if (molstarRef.current && molstarRef.current.jumpToFrame) {
        molstarRef.current.jumpToFrame(targetFrame).then((success) => {
          if (success) {
            // Additional verification: Check what Molstar is actually showing
            setTimeout(() => {
              const modelIndexInput = document.querySelector('.msp-control-group input[type="number"]');
              if (modelIndexInput) {
                const molstarDisplayedFrame = modelIndexInput.value;
                const expectedMolstarModel = targetFrame + 1; // Molstar uses 1-based indexing
                if (molstarDisplayedFrame !== expectedMolstarModel.toString()) {
                } else {
                }
              }
            }, 300);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className={`p-4 rounded-lg shadow-lg border ${
          isDarkMode 
            ? 'bg-[var(--bg-layer-1)] border-[var(--stroke-default)] text-white' 
        ...
        <div style={{ textAlign: 'center', maxWidth: 420 }}>
          <div style={{ marginBottom: 20, color: 'var(--fg-tertiary)' }}>
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" style={{ margin: '0 auto' }}>
              ...
              <path d="M9 19V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2" />
              <path d="M15 19V5a2 2 0 012-2h2a2 2 0 012 2v4a2 2 0 01-2 2h-2a2 2 0 01-2-2" />
            </svg>
          </div>
          <h3 className="text-heading" style={{ fontSize: 17, marginBottom: 8 }}>No ensemble data yet</h3>
          <p className="text-body" style={{ fontSize: 13, lineHeight: 1.6, marginBottom: 20 }}>
            Generate a protein ensemble to explore conformational space with PCA, contact maps, and 3D structure visualization.
          </p>
          <button
  ...
  return (
    <div 
      ref={containerRef}
      className={`h-full w-full ${isDarkMode ? 'bg-[var(--bg-base)] text-white' : 'bg-white text-gray-900'} overflow-hidden`}
    >
        ...
        <div className="p-6">
          <div className="flex justify-between items-start">
            <div className="flex-1">
              ...
              <button
                ...
                className={`text-sm px-3 py-1 rounded-lg border transition-colors ${
                  isDarkMode 
                    ? 'border-[var(--stroke-default)] hover:bg-[var(--bg-layer-2)] text-gray-300' 
            ...
            <div className={`px-3 py-2 rounded-lg ${
              isDarkMode ? 'bg-[var(--bg-layer-1)] border border-[var(--stroke-default)]' : 'bg-white border border-gray-300'
            }`}>
          ...
          {showMethodDetails && (
            <div className={`mt-3 p-3 rounded border text-xs ${
              isDarkMode ? 'bg-[var(--bg-layer-1)]/50 border-[var(--stroke-default)]' : 'bg-gray-50 border-gray-200'
            }`}>
              <div className="grid md:grid-cols-3 gap-3">
                ...
                <div>
                  ...
                  <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'} leading-tight`}>
                    Ca-Ca distances → <code className={`px-1 rounded text-xs ${isDarkMode ? 'bg-[var(--bg-layer-2)]' : 'bg-gray-200'}`}>exp(-d_ij)</code>. 
              ...
              {/* Minimal Stats */}
              {pcaStats && (
                <div className={`mt-3 pt-2 border-t ${isDarkMode ? 'border-[var(--stroke-default)]' : 'border-gray-300'}`}>
          ...
          <div className="flex-1 min-h-[600px] overflow-auto p-2 md:p-4
            ...
            `}>
              <div className={`h-full rounded-lg border ${
                isDarkMode ? 'bg-[var(--bg-layer-1)] border-[var(--stroke-default)]' : 'bg-white border-gray-300'
              }`}>
                <div className="p-2 border-b border-gray-300 dark:border-[var(--stroke-default)]">
                      ...
                      {pcaStats && (
                        <>
                          <div className={`px-2 py-1 rounded ${
                            pcaStats.cumulativeVarianceExplained > 70 
                          ...
                          <div className={`px-2 py-1 rounded ${
                            pcaStats.dataCoverage > 0.95 
                              ? isDarkMode ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-100 text-blue-700'
                              : isDarkMode ? 'bg-[var(--bg-layer-2)] text-gray-400' : 'bg-gray-100 text-gray-700'
                          }`}>
                            {pcaStats.nFrames}/{pcaStats.totalFrames} frames valid
                          </div>
                          <div className={`px-2 py-1 rounded ${isDarkMode ? 'bg-[var(--bg-layer-2)] text-gray-400' : 'bg-gray-100 text-gray-700'}`}>
                  ...
                  {pcaData && Array.isArray(pcaData) && pcaData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        <CartesianGrid 
                          strokeDasharray="3 3" 
                          stroke={isDarkMode ? 'rgba(255,255,255,0.06)' : '#e5e7eb'} 
                        />
                        <XAxis 
                          type="number" 
                          dataKey="pc1" 
            ...
            `}>
              <div className={`h-full rounded-lg border flex flex-col ${
                ...
                <div className="p-2 border-b border-gray-300 dark:border-[var(--stroke-default)] flex-shrink-0">
                ...
                <div className="flex-1 min-h-0 p-3 flex items-center justify-center">
                  {analysisData?.ca_distance_matrices_per_frame && analysisData.ca_distance_matrices_per_frame.length > selectedFrame ? (
                    ...
                    <div className={`h-full rounded ${isDarkMode ? 'bg-[var(--bg-base)]' : 'bg-gray-50'} flex items-center justify-center`}>
            ...
            `}>
              <div className={`h-full rounded-lg border flex flex-col ${
                ...
                <div className="p-3 border-b border-gray-300 dark:border-[var(--stroke-default)] flex-shrink-0">
