...
const SecondaryStructureVisualization = ({ 
  ...
  if (!structureData.length) {
    return (
      <div className={`rounded-lg p-6 ${isDarkMode ? 'bg-[var(--bg-layer-1)]' : 'bg-white border border-gray-200'}`}>
  ...
  const plotColors = {
    ...
    background: chartStyle === 'publication' ? '#FFFFFF' : (isDarkMode ? '#1b1b1f' : '#FFFFFF'),
    text: chartStyle === 'publication' ? '#000000' : (isDarkMode ? '#F3F4F6' : '#212121'),
    grid: chartStyle === 'publication' ? '#606060' : (isDarkMode ? 'rgba(255,255,255,0.06)' : '#E0E0E0')
  };

  return (
    <div className={`p-6 ${isDarkMode ? 'bg-[var(--bg-layer-1)]' : 'bg-white'}`}>
      {/* World-class professional header with progressive disclosure */}
      <div className={`mb-8 rounded-xl overflow-hidden shadow-sm ${isDarkMode ? 'bg-gradient-to-r from-[var(--bg-layer-1)] to-[var(--bg-layer-2)] border border-[var(--stroke-default)]' : 'bg-gradient-to-r from-white to-gray-50 border border-gray-200'}`}>
        {/* Primary header strip */}
        <div className={`px-6 py-4 border-b ${isDarkMode ? 'border-[var(--stroke-default)] bg-[var(--bg-layer-1)]/90' : 'border-gray-100 bg-white/90'}`}>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-3 px-4 py-2 rounded-lg bg-white/5 border border-white/10">
                <select
                  ...
                  className={`px-3 py-1.5 border-0 rounded-lg text-sm font-medium focus:ring-2 focus:ring-blue-500 ${isDarkMode ? 'bg-[var(--bg-layer-2)] text-white' : 'bg-white text-gray-900 shadow-sm'}`}
                >
                  ...
                  <button
                    ...
                    className={`px-4 py-1.5 text-sm font-medium rounded-lg transition-all duration-200 ${
                      showStacked
                        ...
                        : `${isDarkMode ? 'bg-[var(--bg-layer-2)] text-gray-200 hover:bg-[var(--bg-layer-3)] hover:shadow-md' : 'bg-white text-gray-700 hover:bg-gray-50 shadow-sm hover:shadow-md'}`
                    }`}
                  >
        ...
        <details className="group" open>
          ...
          <div className={`px-6 pb-4 border-t ${isDarkMode ? 'border-[var(--stroke-default)] bg-[var(--bg-layer-1)]/50' : 'border-gray-100 bg-gray-50/50'}`}>
      ...
      {showReferenceOverlay && (
        <div className={`mt-4 p-3 border rounded ${isDarkMode ? 'border-[var(--stroke-default)] bg-[var(--bg-layer-2)]' : 'border-gray-300 bg-gray-50'}`}>
          <div className="flex items-center space-x-3 mb-2">
            <select
              value={referenceType}
              onChange={(e) => setReferenceType(e.target.value)}
              className={`px-2 py-1 border rounded text-xs ${isDarkMode ? 'bg-[var(--bg-layer-2)] border-[var(--stroke-default)] text-white' : 'bg-white border-gray-300'}`}
            >
            ...
            <input
              ...
              className={`px-2 py-1 border rounded text-xs flex-1 max-w-24 ${isDarkMode ? 'bg-[var(--bg-layer-2)] border-[var(--stroke-default)] text-white' : 'bg-white border-gray-300'}`}
            />
            ...
            <button
              ...
              className={`px-3 py-1 text-xs font-medium rounded transition-colors ${
                isLoadingReference || !referenceId.trim()
