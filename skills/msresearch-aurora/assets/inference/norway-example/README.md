...
| `frontend/src/App.tsx` | React/Fluent UI viewer with observation ↔ prediction toggle, dark mode, zoom controls, and (Lon,Lat) popovers. Fetches forecast data as JSON at runtime from `frontend/public/data/`. |

Generated forecast payloads (`auroraForecast.json`, `auroraForecastPredictions.json`) live in `frontend/public/data/` and are gitignored to keep the repo small. Follow the regeneration commands below to rebuild them locally.

> **Planning a new region?** Use `python ../../scripts/setup_region.py --name "Your Region" --lat-min X --lat-max Y --lon-min A --lon-max B` to automatically create a working prototype with ERA5 data and adjusted bounds. See [docs/customization-guide.md](../../docs/customization-guide.md) for full instructions including CDS credential setup.
...
Optional: rebuild the observations JSON ahead of inference.

```bash
# 2. (Optional) Rebuild observations JSON
python3 scripts/build_forecast_module.py \
  data/norway_surface.nc \
  --output frontend/public/data/auroraForecast.json
```
...
# 4. Convert predictions to JSON for the UI
python3 scripts/build_forecast_module.py \
  data/norway_june8_forecast.nc \
  --output frontend/public/data/auroraForecastPredictions.json
```

Refresh the browser once `auroraForecastPredictions.json` is regenerated to view the new forecast.

## Data Sources

All datasets are downloaded via the ERA5 APIs using `assets/inference/scripts/download_era5_subset.py`.
...
See `assets/inference/scripts/README.md` for the download script and exact commands that reproduce these files.
