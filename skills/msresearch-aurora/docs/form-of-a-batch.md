...
Aurora’s models expect inputs formatted as an `aurora.Batch`. This guide details the tensor shapes, variable names, and metadata conventions you must follow. Use it alongside the sample inputs in `starter-code/tests/inputs/` for concrete references.
...
- [Surface variables (`Batch.surf_vars`)](#surface-variables-batchsurf_vars)
- [Static variables (`Batch.static_vars`)](#static-variables-batchstatic_vars)
- [Atmospheric variables (`Batch.atmos_vars`)](#atmospheric-variables-batchatmos_vars)
- [Metadata (`Batch.metadata`)](#metadata-batchmetadata)
...
`Batch` contains four components:

1. Surface-level variables.
2. Static variables.
3. Atmospheric variables (all sharing one pressure-level set).
4. Metadata describing coordinates, pressure levels, and timestamps.

All tensors must be **unnormalized**; the model applies normalization internally using statistics from `aurora.normalisation` (mirrored in `starter-code/src/vibe_tune_aurora/data/default_stats.py`).
...
Refer to `starter-code/tests/inputs/` (e.g., ERA5 tiles and `.npz` files) for real datasets that follow this structure.

## Surface variables (`Batch.surf_vars`)
...
For [Aurora 0.4° Air Pollution](available-models.md#aurora-04-air-pollution), the following surface-level variables are
also allowed:
...
For [Aurora 0.25° Wave](available-models.md#aurora-025-wave), the following surface-level variables are also allowed:
...
## Static variables (`Batch.static_vars`)
...
[Aurora 0.4° Air Pollution](available-models.md#aurora-04-air-pollution)
and [Aurora 0.25° Wave](available-models.md#aurora-025-wave) require additional static variables, but these are not
easy to obtain yourself.
You need to obtain these from the HuggingFace repository.
See the description of the models.

## Atmospheric variables (`Batch.atmos_vars`)
...
For [Aurora 0.4° Air Pollution](available-models.md#aurora-04-air-pollution), the following atmospheric variables are
also allowed:
