  ...
  useImperativeHandle(ref, () => ({
    jumpToFrame: async (frameIndex) => {
      if (!trajectoryReady || !viewerInstanceRef.current) {
        console.warn('⚠️ Trajectory not ready for frame control');
        return false;
      }
      ...
      try {
        ...
        try {
          ...
          if (modelIndexInput) {
            ...
            if (frameDifference > 0 && nextButton) {
              ...
              console.warn('⚠️ Navigation buttons not found or invalid frame difference');
              return false;
            }
            ...
            if (finalFrame === targetMolstarFrame) {
              ...
              console.warn(`⚠️ Frame navigation mismatch: expected ${targetMolstarFrame}, got ${finalFrame}`);
              return false;
            }
          }
        ...
        console.warn('⚠️ All frame control methods failed');
        return false;
      ...
      if (ref && ref.current && ref.current.jumpToFrame) {
        ref.current.jumpToFrame(targetFrame).then((success) => {
          if (success) {
            ...
            console.warn(`⚠️ Failed to sync to frame ${targetFrame}`);
          }
        }).catch((error) => {
